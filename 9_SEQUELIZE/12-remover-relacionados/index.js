...
app.post('/users/update', async (req, res) => {
    try {
        const { id, name, occupation, newletter } = req.body;
        
        // Converter o valor do checkbox para booleano
        const newsletterBool = newletter === 'on';
        
        const userData = {
            name,
            occupation,
            newletter: newsletterBool
        };
        ...
        if (updatedRows === 0) {
            console.log('Nenhum usuário foi atualizado. Verifique se o ID está correto.');
            return res.status(400).send("Erro: ID inválido ou dados não alterados.");
        }
...
//Rota para exluir itens relacionados
